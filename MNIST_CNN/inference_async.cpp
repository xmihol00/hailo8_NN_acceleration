...
void runInference(ConfiguredNetworkGroup &model, vector<InputVStream> &inputStreams, vector<OutputVStream> &outputStreams)
{
    ...
    int inputCount = 0;
    InputStream::TransferDoneCallback inputDoneCallback = [&inputStream, &samples, &inputCount](const InputStream::CompletionInfo &completion_info)
    {
        if (completion_info.status != HAILO_SUCCESS)
        {
            throw runtime_error("Failed to write input stream.");
        }
        if (++inputCount < NUM_SAMPLES)
        {
            hailo_status status = inputStream.write_async(MemoryView(samples.data() + inputCount * INPUT_SIZE, INPUT_SIZE * sizeof(uint8_t)));
            if (status != HAILO_SUCCESS && status != HAILO_STREAM_ABORT)
            {
                throw runtime_error("Failed to write input stream.");
            }
            cerr << "Input data " << inputCount + 1 << " written." << endl;
        }
    };
